...
const RowOfService = ({ service }) => {
    const { title, desc, sortName } = service
    return (
        <div>
            <div class="w-full max-w-sm px-4 py-3  rounded-md shadow-md bg-gray-800">
                <div class="flex items-center justify-between">
                    <span class="text-sm font-light text-gray-400">Offers & Bounes</span>
                    <span class="px-3 py-1 text-xs  uppercase rounded-full bg-blue-300 text-blue-900">{sortName}</span>
                </div>

                <div>
                    <h1 class="mt-2 text-lg font-semibold text-white">{title}</h1>
                    <p class="mt-2 text-sm text-gray-300">{desc}</p>
                </div>

                <div>
                    <div class="flex items-center mt-2 text-gray-200">
                        <span>Visit on:</span>
                        <a class="mx-2  cursor-pointer text-blue-400 hover:underline" tabindex="0" role="link">hero.org</a>
                        <span>or</span>
                        <a class="mx-2 cursor-pointer text-blue-400 hover:underline" tabindex="0" role="link">herorider.com</a>
                    </div>

                    <div class="flex items-center justify-center mt-4">
                        <a class="mr-2  cursor-pointer text-gray-400 hover:text-gray-700 dark:hover:text-gray-300" tabindex="0" role="link" aria-label="twitter link">
                        ...
                        <a class="mr-2 cursor-pointer text-gray-400 hover:text-gray-700 hover:text-gray-300" tabindex="0" role="link" aria-label="share link">
